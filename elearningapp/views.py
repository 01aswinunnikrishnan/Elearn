...
def mearnstack_home(request):
    return render(request, 'mernhm.html')
...
def trainer_view(request):
    ...
    return render(request, 'admin1.html', {'data': d})

def del_trainer(request,id):
    trainer_reg.objects.filter(pk=id).delete()
    return redirect(trainer_view)

def accept_trainer(request,id):
    trainer_reg.objects.filter(pk=id).update(status='approve')
    return redirect(trainer_view)
